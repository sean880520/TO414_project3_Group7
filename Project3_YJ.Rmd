...
## Preparing final test dataset for prediction

```{r}
#cleaning test set in the same way as the training set
...
library(VIM)
library(mice)
mice_plot <- aggr(houses_test, col=c('navyblue','yellow'),
                    numbers=TRUE, sortVars=TRUE,
                    labels=names(houses_test), cex.axis=.4,
                    gap=3, ylab=c("Missing data","Pattern"))

#Dropping all variables with more than 15% of missing data as they might lead to incorrect and unbalanced imputation
drop <- c("PoolQC", "MiscFeature","Alley","Fence", "FireplaceQu", "LotFrontage")
houses_test <- houses_test[,!names(houses_test) %in% drop]


#drop rows with NAs in these columns, becasue the NAs here make up less than 6% of total data, and imputation will not be as accurate as there is overwhelmingly large amount of a single type of data in that column. 
houses_test <- houses_test[!is.na(houses_test$MasVnrType),]
houses_test <- houses_test[!is.na(houses_test$MasVnrArea),]
houses_test <- houses_test[!is.na(houses_test$GarageType),]
houses_test <- houses_test[!is.na(houses_test$BsmtQual),]
houses_test <- houses_test[!is.na(houses_test$BsmtExposure),]
houses_test <- houses_test[!is.na(houses_test$BsmtFinType2),]
houses_test <- houses_test[!is.na(houses_test$Electrical),] #only one missing observation

#converting all categorical data to numeric data
houses_test[] <- data.matrix(houses_test)

#Imputing the NAs for the rest of the variables using predicitve mean matching
houses_test <- mice(houses_test, m=5, maxit = 50, method = 'pmm', seed = 100)
houses_test <- complete(houses_test)
write.csv(houses_test, "CleanedImputed_Test.csv") #took a while to run this imputation code, so I created a new file so that there's no need to run this code again unless necessary

# Select only the features that passed the boruta screening
cleaned_test <- read.csv("CleanedImputed_Test.csv")
houses_test <- cleaned_test[getSelectedAttributes(boruta_Train)]
...
#no need exclude the SalePrice from normalization as there is no SalePrice column in the test set
houses_test <- as.data.frame(lapply(houses_test, normalize))
```
...
# Preparing the in-sample train and test datasets

**model doesn't work - not sure why for now**
```{r}
#Step 1: Randomise the data
set.seed(65)
houses_random <- houses_train[sample(nrow(houses_train)), ]

train_insample_ANN <- houses_random[1:1000,]
test_insample_ANN <- houses_random[1001:1338,]

library(neuralnet)
houses_model <- neuralnet(formula = SalePrice ~. , data = train_insample_ANN, hidden=2,  linear.output=T, stepmax = 1000000)

# obtain model results
model_results <- compute(houses_model, test_insample_ANN[,!names(houses_random)=="SalePrice"])
# obtain predicted strength values
predicted_saleprice <- model_results$net.result
summary(predicted_saleprice)
# examine the correlation between predicted and actual values
cor(predicted_saleprice, test_insample_ANN$SalePrice)
```
