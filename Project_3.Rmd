...
svm_train <- cleanHouseData_Train[1:1020, ]
svm_test  <- cleanHouseData_Train[1021:1459, ]

# Training a SVM model (kernal "rbfdot" produces the lowerst rmse after trials)
library(kernlab)
...
model_svm2 <- ksvm(SalePrice ~ MSZoning+ LotArea+ LotShape+ LandContour+ LandSlope+ Neighborhood+ BldgType+ OverallQual+ OverallCond+ Exterior1st+ Exterior2nd+ ExterQual+ BsmtQual+ BsmtExposure+ BsmtFinType1+ BsmtFullBath+ FullBath+ KitchenQual+ TotRmsAbvGrd+ Functional, data = svm_train, kernel = "rbfdot")
...
rmse_2 = (se/nrow(prediction_svm1))^0.5
rmse_2
```
