...
set.seed(333)
fit <- train(SalePrice ~., data = training,tuneGrid = expand.grid(k=1:70), method = 'knn', trControl = trControl,preProc = c('center', 'scale'))
...
cor(pred,test$SalePrice)

```

The KNN Regression we constructed helps in determining which explanatory variables are best in predicting the sale price for a house. The varImp function lets us see that OverallQual,GrLivArea, and TotalBsmtSF are some of the best variables in price the sale price of a house. Meanwhile, variables like the number of fireplaces, or the year the garage was built are not that valuable in this prediction. Furthemore, the plot above shows the actual  sale price of a house against the predicted sale price of the house. Since the plot is pretty linear - with the exception of a few outliers - and has a slope close to one, we believe this is a decent model in predicting the sale price of a house.  
