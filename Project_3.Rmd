...
The ANN Model predictions and the actual house prices have a correlation of `r round(cor(ANN_Prediction, ANN_HouseData_Test$SalePrice), digits = 3)`. This correlation is approximately 0.037 higher than the correlation obtained when all the variables from Boruta Screening were used.
...
The house prices predicted by the SVR Model that uses all the variables that passed the Boruta Screening and the rbfdot kernel, and the actual house prices have a correlation of `r round(cor(SVR_Prediction, SVR_HouseData_Test$SalePrice), digits = 3)`.
...
The kNN Model predictions and the actual house prices have a correlation of `r round(cor(kNN_Prediction, kNN_HouseData_Test$SalePrice), digits = 3)`, which supports the indication of the plot that the kNN Model is a decent predictor for house prices. 
...
The house prices predicted by our combined model and the actual house prices have a correlation of `r round(cor(House_Prices$`Weighted Average`, House_Prices$`True Price`), digits = 3)`. Our combined model beats all our other models in accuracy, besides the SVR Model, for which the correlation between predicted and actual prices was `r round(cor(SVR_Prediction, SVR_HouseData_Test$SalePrice), digits = 3)`.
...
Based on the cleaned data, we then went on and built four different predictive models, including a Random Forest Model, an Artificial Neural Network Model, a Support Vector Machine Model, and a k-Nearest-Neighbors Model. **Out of the predictions of the four models, the predicted prices produced with the SVR Model had the highest correlation with the actual house prices listed in our Test Dataset. The correlation between the SVR Model Predictions and the actual house prices was `r round(SVR_Cor, digits = 4)`.**
...
After creating the four individual models, we still thought that we could improve our predictions by obtaining **a correlation-weighted average of the predictions** of the individual models to create a combined prediction model that would then be trained across multiple mechanisms. **The correlation between the price predictions made with our combined model and the actual house prices was `r round(CombinedModel_Cor, digits = 4)`. This correlation (accuracy) was the second best between all the models.** 
...
# Executing Boruta Training
set.seed(233)
boruta_Train2 <- Boruta(rawHouseData_Test[names(rawHouseData_Test) != "Id" & names(rawHouseData_Test) != "SalePrice" ], rawHouseData_Test$SalePrice ,  doTrace = 2)
final.boruta2 <- TentativeRoughFix(boruta_Train2)
print(final.boruta2)
plot(final.boruta2)

# Selecting only the features that passed the Boruta Screening
cleanHouseData_Test <- rawHouseData_Test[getSelectedAttributes(final.boruta2, withTentative = F)]
...
```{r}
Final_Kaggle_Prediction <- predict(SVR_Model, clean_HouseData_Test)
```
