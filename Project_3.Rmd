...
We built two combined models, one of which uses correlation as weights and and the other uses RMSE as weights. Then, we did a weighted-average for each model to produce the final prediction from all four models. It is worth noting here that the second combined model used RMSE of each model to conduct an inverse weighting scheme that yielded the results, since a smaller RMSE means a better fit. 
...
Based on the cleaned data, we then went on and built four different predictive models, including a Random Forest Model, an Artificial Neural Network Model, a Support Vector Machine Model, and a k-Nearest-Neighbors Model. **Out of the predictions of the four models, the predicted prices produced with the SVR Model had the highest correlation with the actual house prices listed in our Test Dataset. The correlation between the SVR Model Predictions and the actual house prices was `r round(SVR_Cor, digits = 4)`. Furthermore, it also has the lowest RMSE among all models which is `r round(SVR_RMSE, digits = 4)`**
...
### Making Test Dataset in the Same Format as the Train Data
