...
To decide the variable combination used in the Random Forest Model, we tested 2 possible combinations of variables. The first combination of variables will simply include all variables in RF_HouseData_Train, which includes all the variables deemed as important by Boruta. The second combination of variables will include all the significant variables with p value lower than 0.05 from the final stepwise regression model. The reason for taking the variables from the final stepwise regression model is because the final stepwise regression model is the most optimised with the lowest AIC. 

In our code, we have chosen to only show the second combination of variables from stepwise regression as it produces the highest accuracy, which was measured by the strength of correlation. 

```{r}
RF_Model <- randomForest(SalePrice ~ Neighborhood + HouseStyle + MasVnrArea + FullBath + MSZoning + Exterior1st + BsmtFinType1 + X2ndFlrSF + BedroomAbvGr + TotRmsAbvGrd + Functional + LotFrontage + LandContour + LandSlope + Condition1 + YearBuilt + ExterQual + KitchenAbvGr + KitchenQual + Fireplaces + WoodDeckSF + SaleCondition + LotArea + LotShape + BldgType + OverallQual + OverallCond + BsmtQual + GrLivArea + BsmtFullBath + KitchenQual + + GarageCars + GarageQual + GarageCond, data = RF_HouseData_Train)
RF_Model
...
There is a strong correlation of `r cor(RF_Prediction,RF_HouseData_Test$SalePrice), digits = 3)` between the house prices predicted by the Random Forest Model and actual prices. This implies a good prediction and that the predicted prices follow a similar trend with the real prices. This, compared to the correlation when all the variables from Boruta screening were used, is higher by 0.002689.
...
The ANN Model predictions and actual house prices have a correlation of `r round(cor(ANN_Prediction, SVM_HouseData_Test$SalePrice), digits = 3)`. This correlation was obtained using the significant variables from the stepwise regression and is 0.0370928 higher than the correlation obtained when all the variables from Boruta screening was used.
...
For the SVM Model, we first used all the variables from Boruta screening to construct the SVM model and optimise for the best kernel. Next, using the kernel with the highest accuracy, we tested the 2 variable combinations as mentioned above. 
...
```{r}
# Constructing SVM Model with Boruta sc
...
With the Boruta screening variables, the SVM Model prediction (using the rbfdot kernel) and actual house prices have a correlation of `r round(cor(SVM_Prediction, SVM_HouseData_Test$SalePrice), digits = 3)`, which is much higher than the other 3 kernels. 
